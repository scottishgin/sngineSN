...
try {
	...
	switch ($_GET['view']) {
		case '':
			...
			// get articles
			$articles = $user->get_articles(['results' => 11]); /* using 11 to match the design layout */
			/* assign variables */
		...
		case 'category':
			...
			// page header
			page_header($system['system_title'].' - '.__("Blogs").' - '.__($category['category_name']), __($category['category_description']));
